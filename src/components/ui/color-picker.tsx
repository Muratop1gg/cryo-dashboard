...
export const ColorPickerHue = ({ className, ...props }: ColorPickerHueProps) => {
  ...
  return (
    <Slider.Root
      className={cn("relative flex w-4 h-full touch-none", className)}
      max={360}
      ...
      {...(props as any)}
    >
      <Slider.Track className="relative my-0.5 w-4 h-full grow rounded-full bg-[linear-gradient(0deg,#FF0000,#FFFF00,#00FF00,#00FFFF,#0000FF,#FF00FF,#FF0000)]">
...
export const ColorPickerOutput = ({ className, ...props }: ColorPickerOutputProps) => {
  ...
  // Обёртка для обработки значения от Select
...
export const ColorPickerFormat = ({ className, ...props }: ColorPickerFormatProps) => {
  ...
  if (mode === "hex") {
    ...
    return (
      <div
        className={cn(
          "-space-x-px relative flex w-34 items-center rounded-md shadow-sm",
          className,
        )}
        {...(props as any)}
      >
        <Input
          className="h-8 bg-white/20 border border-white/18 rounded-lg px-2 text-xs shadow-none"
          readOnly
          type="text"
          value={hex}
        ></Input>
        <div className="w-12 h-6 rounded-md absolute left-21" style={{ backgroundColor: hex }}></div>
